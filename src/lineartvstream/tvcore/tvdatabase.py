import logging
from sqlalchemy import Column, Integer, Boolean, Float, Date, DateTime, ForeignKey, Text, create_engine, update, and_, or_, case, func, desc, text, inspect
from sqlalchemy.orm import relationship, sessionmaker, Session, DeclarativeBase, Mapped
...
class Channels(Base):
    ...
    id: Mapped[int] = Column(Integer, primary_key=True)
    channel_id: Mapped[str] = Column(Text, nullable=False, unique=True)
...
class TVDatabase:
    ...
    def _execute(self, query, model = None, first=False):
        with self.get_session() as session:
            try:
                if first:
            ...
            if model is None:
                return row if first else results
    ...
    def get_schedule(self, schedule_id = None, date: datetime = None, channel: str = None, episode_id=None, movie_id=None) -> List[ScheduleOutput]:
        ...
        if channel:
            q = q.where(Schedule.channel == channel)
        if episode_id:
            q = q.where(Schedule.episode_id == episode_id)
        if movie_id:
            q = q.where(Schedule.movie_id == movie_id)

        return self._execute(q, ScheduleOutput)


    def bulk_update_schedule(self, episode_id, duration):
        with self.get_session() as session:
            rows = session.execute(
                select(Schedule.id, Schedule.start)
                .where(Schedule.episode_id == episode_id)
            ).all()
            ...
            updates = [
                {
                    "id": row.id,
                    "end": row.start + timedelta(seconds=duration)
                }
                for row in rows
            ]

            session.execute(update(Schedule), updates)
            session.commit()
    
    def get_schedule_conflict(self, channel: str, start: datetime, end: datetime) -> List[ScheduleOutput]:
        """Get all programs that are in the weekly schedule"""
        q = select(
            Schedule
        ).where(
    ...
    def get_current_program(self, channel: str, time=None) -> list[dict]:
        if time is None:
            time = datetime.now()
        ...
        with self.get_session() as session:
            ...
            return session.execute(q).mappings().first()
            
    def get_next_program(self, channel:str, time:datetime=None, limit=1) -> list[dict]:
        if time is None:
            time = datetime.now()
